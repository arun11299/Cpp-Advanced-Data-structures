...
{
  assert (node);
  if (node->left_subtree_ht() >= node->right_subtree_ht()) {
...
template <typename T, typename Comp>
auto
AVLTree_Base<T, Comp>::do_left_left_rotate(NodeTypePtr node)
  -> NodeTypePtr
{
  assert (node);
...
template <typename T, typename Comp>
auto 
AVLTree_Base<T, Comp>::do_left_right_rotate(NodeTypePtr node)
  -> NodeTypePtr
{
  assert (node);
  auto tmp = node->left();
